...
static inline __noreturn void call(struct pt_regs *regs, unsigned int nr){
...
//questi leggermente modificati
//cambiato nome, non più init_module del modulo
...
//se sto in versioni moderne devi guardare qui dentro
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)

	//sfrutto il meccanismo per ottenere la funzione con il grosso switch dispatcher
	if (register_kprobe(&kp_x64_sys_call)) {
                printk(KERN_ERR "%s: cannot register kprobe for x64_sys_call\n", MODNAME);
                return -1;
        }
        //qui salvo proprio questo indirizzo
        ...
        //qui si prepara il contenuto per binary patching
